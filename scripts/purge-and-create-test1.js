...
async function main() {
  ...
  console.log('Purging legacy corrupted test accounts via RPC...');
  const { data: purgeData, error: purgeErr } = await client.rpc('admin_purge_test_accounts');
  console.log('Purge Result:', purgeData, purgeErr);

  // Now create clean employee 'test1' with password 'Ravva@1234'
  console.log('\n2. Creating fresh employee username "test1" with password "Ravva@1234"...');
